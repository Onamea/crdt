...
import { isSubKeyDomain } from "./Identifier.js";
import { isId } from "./Identity.js";
import { createCreateOperation, createDeleteOperation, createDenounceOperation, createGrantOperation, createRelateOperation, createRevertOperation, createRevokeOperation, createSetOperation, createUnrelateOperation, createValidateOperation, createVouchOperation } from "./Operation.js";
import isString from "./lib/utils/isString.js";
const delimiter = "\n";
const joinWithDelimiter = (parts) => {
    return parts.filter(part => part !== undefined).join(delimiter);
};
const splitByDelimiter = (str) => {
    return str.split(delimiter);
};
export const parseRawOperation = async (rawOperation) => {
    ...
    const operationType = operationTypeString !== undefined ? parseInt(operationTypeString, 10) : undefined;
...
export const isRawOperation = async (value) => {
...
export const toRawOperation = (operation) => {
    const { id, type } = operation;
    if (type === "CREATE") {
        return id;
    }
    const { previousHash } = operation;
    if (type === "SET") {
